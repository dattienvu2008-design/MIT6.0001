...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    ciphertext_obj = ps4b.EncryptedMessage(ciphertext.message_text)
    most_reliable_decrypt_message = ''
    valid_word_count = 0
    max_valid_word_count = 0
    best_pad = []
    for pad in pads:
        ciphertext_temp_list = ciphertext_obj.decrypt_message(pad.copy()).message_text.split()
        for word in ciphertext_temp_list:
            if is_word(word_list, word) == True:
                valid_word_count += 1
        if valid_word_count > max_valid_word_count:
            max_valid_word_count = valid_word_count
            best_pad = pad.copy()
            most_reliable_decrypt_message = ciphertext_obj.decrypt_message(pad.copy()).message_text
    if max_valid_word_count == 0:
        best_pad = pads[-1].copy()
        most_reliable_decrypt_message = ciphertext_obj.decrypt_message(pads[-1].copy()).message_text
    return ps4b.PlaintextMessage(most_reliable_decrypt_message,best_pad)
